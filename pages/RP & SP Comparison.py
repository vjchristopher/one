...
st.subheader('🆚 Reserve Price and Selling Price compared in various auctions :', divider='grey')
        ...
        for x,y in enumerate(df[col]): # x=0, y=RP; x=1,y=SP          
            ...
            if x==0: #x=0 corresponding to RP
                ...
                if y==-1:
                    ...
                    fig.add_annotation(x=x,y=-1,text=f' No Bids recevd for LSAs in <br>{coly1} and<br>{coly2} ',showarrow=False,
...
if submitted: #The submit button has been pressed.  
    ...
    band=read_bands()
    st.markdown(f":blue[Sub: Frequency band {band}  MHz Bid outcome from  {year}  auction.]")
